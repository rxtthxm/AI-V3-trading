...
def run_bot():
    ...
    try:
        ...
        print(f"❌ ดึงข้อมูลบัญชีไม่สำเร็จ: {e}")
        return
    ...
    # Handling potential MultiIndex or missing columns from yfinance
    macro_data = pd.DataFrame(index=macro_raw.index)
    macro_data['VIX'] = macro_raw['^VIX']
    macro_data['Yield10Y'] = macro_raw['^TNX']
    macro_data['SPY'] = macro_raw['SPY']
    
    macro_data['VIX_ROC'] = macro_data['VIX'].pct_change(5)
    macro_data['SPY_MOM_5'] = macro_data['SPY'].pct_change(5)
    macro_data['SPY_MOM_20'] = macro_data['SPY'].pct_change(20)
    macro_data['SPY_Regime'] = (macro_data['SPY'].pct_change(60) > 0).astype(int)
